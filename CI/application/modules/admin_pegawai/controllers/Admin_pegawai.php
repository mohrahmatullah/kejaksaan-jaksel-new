...
class Admin_pegawai extends CI_Controller
{
	...
	public function get_opsi_kabupaten($prop)
	{
		...
		echo '<select id="kabpegawai" class="form-sprint select2" name="kabpegawai" style="width:100%;">';
	...
	public function get_opsi_nama_atasan($jabatanatasan)
	{
		$ats	= $this->admin_pegawai_model->get_nama_atasan($jabatanatasan)->result_array();
		
		echo '<select id="pegawainamaatasan" class="form-sprint select2" name="pegawainamaatasan" style="width:100%;">';
		echo '<option value="">Pilih Nama Atasan</option>';
		foreach ($ats as $data){
	...
	public function edit_pegawai($id,$page,$cari)
	{
		...
		if($data['custdata']->jabatan_id != 2){
			$data['peg_atasan']		= $this->admin_pegawai_model->get_pegawai_atasan($id);
			$data['peg_jab_atasan']	= $this->admin_pegawai_model->get_pegawai_detail($data['peg_atasan']->parent);
		}
				
		$data['users']			= $this->admin_pegawai_model->get_users()->result_array();
		$data['propinsiall']	= $this->admin_pegawai_model->get_propinsi()->result_array();
	...
	public function lihat_pegawai($id)
	{
		$data['detail_pegawai']		= $this->admin_pegawai_model->get_pegawai_view_detail($id);
	...
	{
		$arr		= $this->admin_pegawai_model->get_pegawai_view_detail($id);
	...
	public function simpan_pegawai()
	{
		...
		if ($id == ''){
			...
			if($this->input->post('jabatan') != 1){
				$this->db->insert('pegawai_chain', ['pegawai_id' => $idlast,'parent' => $this->input->post('pegawainamaatasan')]);
			}
			...
			if($this->input->post('jabatan') != 1){
				$this->db->where('pegawai_id', $id);
				$this->db->update('pegawai_chain', ['parent' => $this->input->post('pegawainamaatasan')]);
			}
