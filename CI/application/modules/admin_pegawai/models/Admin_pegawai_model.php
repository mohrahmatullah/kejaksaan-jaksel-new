...
class Admin_pegawai_model extends CI_Model {
	...
	function get_pegawai_atasan($id){
		$this->db->select("*");
		$this->db->from("pegawai_chain");
	...
	function get_pegawai_view_detail($id){
		$this->db->select("pegawai.*, pegawai_jabatan.nama as nama_jabatan");
		$this->db->from("pegawai");
		$this->db->join("pegawai_jabatan","pegawai.jabatan_id = pegawai_jabatan.id_jabatan","left");
    	$this->db->where('id_pegawai', $id);
		$result = $this->db->get();
	...
	function get_nama_atasan($jabatanatasan){
		$this->db->select("*");
		$this->db->from("pegawai");
    	$this->db->where('jabatan_id', $jabatanatasan);
