...
class Admin_verification extends CI_Controller
{
	...
	public function get_uraian_detail($id){
		$data = $this->admin_verification_model->get_uraian_detail($id);
	...
	public function simpan_verification()
	{
		...
		$this->db->select("verification.id_verification as id");
		$this->db->from("verification");
		$this->db->where('uraian_id', $this->input->post('uraian'));
		$this->db->where('pegawai_id', $this->input->post('pegawai'));
		$result = $this->db->get();
		$rs = $result->row();
		$id = $rs->id;
