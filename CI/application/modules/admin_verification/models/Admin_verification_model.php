...
class Admin_verification_model extends CI_Model {
	...
	function get_uraian_detail($id){
		$this->db->select("uraian.*, uraian.nama as nama_uraian, sop.id_sop, sop.nama as nama_sop, peraturan.id_peraturan, CONCAT(peraturan.nomor, ' | ', peraturan.tahun, ' | ', peraturan.tentang) AS peraturan_lengkap, peraturan.tentang as judul_peraturan, verification.*, uraian.pegawai_id as idpegawai, pegawai.nama as namapegawai");
		$this->db->from("uraian");
		$this->db->join("sop","sop.id_sop = uraian.sop_id","left");
		$this->db->join("peraturan","peraturan.id_peraturan = sop.peraturan_id","left");
		$this->db->join("pegawai","pegawai.id_pegawai = uraian.pegawai_id","left");
