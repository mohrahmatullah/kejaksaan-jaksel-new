		...
		<div class="" style="width: 700px;">
		  <div class="row">
		    <div class="col-12">
		      ...
		      <form id="submitcarisop" method="post" action="" enctype="multipart/form-data" style="position: relative;display: inline-block;width: 80%;">
		    ...
		    <div class="row">
		      ...
		      <div class="col-12 col-lg-6 mb-6">
		        <div class="list-c">
		          ...
		          <a href="javascript:void(0);"  onclick="viewdata(<?= $data['id_sop'] ?>);" class="list-c-nm"><?= $data['nama'] ?></a>
		          <div class="d-flex align-items-center">
		          	<button type="button" onclick="editid(<?=$data["id_sop"] ?>,<?= $hal ?>,'<?= $cari ?>');" title="Edit" class="list-c-mn mr-2"><img class="" src="<?= ASSETS_IMAGE ?>/task_ico.png" style="width: 35px;height: 35px;" /></button>
		          	<a href="javascript:void(0);" href="javascript:void(0);" onclick="if (confirm('Apakah data akan dihapus ?')) deleteid(<?=$data["id_sop"] ?>,<?= $hal ?>,'<?= $cari ?>')" title="Delete" class="list-c-mn mr-2"><img class="" src="<?= ASSETS_IMAGE ?>/trash.png" style="width: 25px;height: 25px;" /></a>
		          </div>              
		        </div>
		      </div>          
			    <?php
  ...
  <section class="mb-3 mt-5 mt-xl-0">
    <div class="row align-items-center">
      <div class="col-7 pr-0">
        <div class="text-uppercase" style="font-size: 12pt;font-weight: bold;color: rgba(0,0,0,.8)">SOP</div>
      </div>
      <div class="col-5 pl-0">
        ...
        <a href="#" id="addsopbtn" onclick="addnew();"><img class="" src="<?= ASSETS_IMAGE ?>/ico_add.png" style="width: 35px;height: auto;" /></a>
      </div>
    </div>    
  </section>
    ...
    <div class="w-100">
      <div class="row">
        <div class="col-12 d-inline-flex align-items-center">
          ...
          <form id="submitcarisopmobile" method="post" action="" enctype="multipart/form-data" style="position: relative;display: inline-block;width: 80%;">
      ...
      <div class="col-12 col-lg-4 mb-4">
        <div class="list-c">
          ...
          <div style="display: flex;">
            <a href="javascript:void(0);" onclick="editid(<?=$data["id_sop"] ?>,<?= $hal ?>,'<?= $cari ?>');" class="list-c-mn d-inline-flex align-items-center"><img class="" src="<?= ASSETS_IMAGE ?>/task_ico.png" style="width: 35px;height: 35px;" /></a> 
            <a href="javascript:void(0);" onclick="if (confirm('Apakah data akan dihapus ?')) deleteid(<?=$data["id_sop"] ?>,<?= $hal ?>,'<?= $cari ?>')" title="Delete" class="list-c-mn d-inline-flex align-items-center"><img class="" src="<?= ASSETS_IMAGE ?>/trash.png" style="width: 25px;height: 25px;" /></a>
          </div>
        </div>
      </div>
        <?php
...
<script>
	...
	function viewdata(id){
		...
		var urlAdd = "<?php echo base_url().'admin_sop/view_sop/'; ?>" + id;
		var dataObj    = {};
    	dataObj.id     =  id;
    	var loop = "";
    	var head = "";
		$.ajax({
          ...
          success: function(data){
          	// console.log(data);
            loop += '<h5 class="mb-3"><strong>'+ data.nama + '</strong></h5><h5 class="mb-1">File Before : <a href="">'+ data.file + '</a></h5>';
	...
	$(document).ready(function() {	
			...
			$('#submitcarisopmobile').submit(function(event) {
		        event.preventDefault();
